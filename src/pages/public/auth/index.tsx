...
export default function AuthPage() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-white">
      {/* ── Left panel ── */}
      <div className="hidden lg:flex lg:w-[420px] xl:w-[460px] flex-col justify-between bg-violet-600 p-10">
        <div>
          {/* Logo */}
          ...
          <div className="space-y-4">
            <h1 className="text-3xl font-bold leading-tight text-white">
              ...
              <span className="text-violet-200">Chat with your docs.</span>
            </h1>
          ...
          <div className="mt-10 space-y-4">
            {[
              ...
              <div key={item.title} className="flex items-start gap-3">
                <div className="mt-0.5 h-5 w-5 flex-shrink-0 rounded-full bg-white/20 flex items-center justify-center">
        ...
        <div className="flex items-center gap-3 rounded-xl border border-white/20 bg-white/10 px-4 py-3">
          ...
          <p className="text-xs text-violet-200">Powered by FastAPI · <span className="text-white">/api</span></p>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col items-center justify-center px-6 py-12 bg-gray-50">
        ...
        <div className="mb-8 flex items-center gap-2 lg:hidden">
          ...
          <span className="text-base font-semibold text-gray-900">FMate</span>
        </div>
        ...
        <div className="w-full max-w-sm">
          ...
          {/* Mode toggle */}
          <div className="mb-6 flex rounded-xl bg-gray-100 border border-gray-200 p-1">
            <button
              type="button"
              onClick={() => setMode("signin")}
              className={`flex-1 rounded-lg py-2 text-sm font-medium transition-colors ${mode === "signin" ? "bg-violet-600 text-white shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
            >
              Sign in
            </button>
            <button
              ...
              className={`flex-1 rounded-lg py-2 text-sm font-medium transition-colors ${mode === "signup" ? "bg-violet-600 text-white shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
            >
              Sign up
            </button>
          </div>
            ...
            <form onSubmit={handleSignIn} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-50 transition-all"
                />
              </div>
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="••••••••"
                    required
                    className="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 pr-10 text-sm text-gray-800 placeholder:text-gray-400 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-50 transition-all"
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                disabled={loading}
                className="mt-2 w-full rounded-xl bg-violet-600 py-2.5 text-sm font-semibold text-white hover:bg-violet-500 disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
              >
